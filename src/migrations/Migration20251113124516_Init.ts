...
export class Migration20251113124516_Init extends Migration {
    override up(): void {
        ...
        this.addSql(
            `create table "chats" ("id" uuid not null default uuid_generate_v4(), "created_at" timestamptz not null default now(), "name" varchar(64) null, "title" varchar(255) null, "count_messages" int not null default 0, "type" "chat_type_enum" not null default 'is_open', "max_users_online" int not null default 1, constraint "chats_pkey" primary key ("id"));`,
        );
        this.addSql(`create index "chats_name_index" on "chats" using hash ("name");`);
        this.addSql(`create index "chats_title_index" on "chats" using GIN ("title");`);

        this.addSql(
            `create table "users" ("id" varchar(64) not null, "name" varchar(32) null, "user_name" varchar(64) not null, "rsa_public_key" varchar(4096) not null, "encrypted_rsa_private_key" varchar(16384) not null, "password_hash" varchar(64) not null, "seed_phrase_hash" varchar(64) not null, "created_at" timestamptz not null default now(), constraint "users_pkey" primary key ("id"));`,
        );

        this.addSql(
            `create table "messages" ("id" uuid not null default uuid_generate_v4(), "created_at" timestamptz not null default now(), "number" int not null, "message" varchar(4096) null, "type" "message_type_enum" null default 'is_user', "chat_id" uuid not null, "user_id" varchar(64) null, "parent_message_id" uuid null, constraint "messages_pkey" primary key ("id"));`,
        );
        ...
        this.addSql(
            `create table "chat_keys" ("id" uuid not null default uuid_generate_v4(), "created_at" timestamptz not null default now(), "encryption_key" varchar(4096) null, "received" boolean not null default false, "chat_id" uuid not null, "user_id" varchar(64) not null, constraint "chat_keys_pkey" primary key ("id"));`,
        );
        this.addSql(
            `create index "chat_keys_user_id_chat_id_index" on "chat_keys" using btree ("user_id", "chat_id");`,
        );
        this.addSql(
            `alter table "chat_keys" add constraint "chat_keys_chat_id_user_id_unique" unique ("chat_id", "user_id");`,
        );
        ...
        this.addSql(
            `alter table "messages" add constraint "messages_user_id_foreign" foreign key ("user_id") references "users" ("id") on update cascade on delete set null;`,
        );
        this.addSql(
        ...
        this.addSql(
            `alter table "chat_keys" add constraint "chat_keys_user_id_foreign" foreign key ("user_id") references "users" ("id") on update cascade;`,
        );
    }
    ...
    override down(): void {
        ...
        this.addSql(`alter table "messages" drop constraint "messages_user_id_foreign";`);

        this.addSql(`alter table "chat_keys" drop constraint "chat_keys_user_id_foreign";`);
